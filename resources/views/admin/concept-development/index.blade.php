							...
							<div class="col-12">
								<div style="float: left;">
									<button
										...
										onclick="window.location.href='admin-concept-development';"
						...
						<tbody>
							...
							<tr>
								...
								<td style="white-space: nowrap;">
									{{ $list->concept_dev_code }}
								</td>
								<td>
									<div>{{ $list->concept_dev_name_th }}</div>
									<div>{{ $list->concept_dev_name_en }}</div>
								</td>
								<td>
								...
								<td>
									@if ( $list->concept_dev_model_file_path1 != "" )
									<a href="{{ $front_base }}uploads/{{ $list->concept_dev_model_file_path1 }}" target="_blank">
										{{ $list->concept_dev_model_file_name1 }}
									</a>
									@endif
								</td>
								...
								<td style="min-width: 240px;">
									<select
										class="form-control"
										onchange="StatusSelectChange( $(this), {{ $list->concept_dev_id }} )"
									>
										@foreach ($rs_status as $status)
											@if( $status->type_sub_id == $list->concept_type_sub_id )
											<option
												value="{{ $status->type_status_id }}"
												@if( $status->type_status_id == $list->concept_type_status_id ) selected @endif
											>
								...
								<td>
									...
									<div style="display: flex; justify-content: center; align-items: center;">
										...
										<button
											...
											onclick="InfoShow( {{ $list->concept_dev_id }} )"
...
<!-- InfoModal -->
<div class="modal fade" id="InfoModal" tabindex="-1" data-backdrop="static" size="big" aria-hidden="true">
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">คอนเซ็ปท์งานวิจัย</h5>
...
<script>
	...
	function InfoShow(id){
		...
		$.ajax({
			type: "GET",
			url: "admin-concept-development/"+id,
			cache: false,
